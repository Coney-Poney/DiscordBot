	...
	async execute(interaction) {
		...
		if (interaction.options.getBoolean("public") == undefined || interaction.options.getBoolean("public") == false) {
			await interaction.reply({
				content: "general/getString".getLang().format(interaction.options.getString("string", true), this._Module._Client.LanguageMananger.language, interaction.options.getString("string", true).split("/").join(" -> "), string),
				ephemeral: true
			});
		} else {
			await interaction.reply({
